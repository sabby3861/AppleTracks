...
protocol ATActivityViewProtocol {
    func showActivityIndicatory(view: UIView, _ title: String)
    func removeActivity()
}

class ATActivityView: ATActivityViewProtocol {
...
protocol ATAlertDelegate {
    func alert(buttonClickedIndex:Int, buttonTitle: String, tag: Int)
}
class ATAlertViewController {
    class func showAlert(withTitle title: String, message:String, buttons:[String] = ["Ok"], delegate: ATAlertDelegate? = nil, tag: Int = 0){
